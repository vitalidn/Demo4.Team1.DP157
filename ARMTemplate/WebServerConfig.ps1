		...
		Script RemoveDefaultWebSite
		  {
			...
			TestScript = {
			  if ((Get-Website -Name "Default Web Site").Name -eq "Default Web Site") {
			    #remove the site
				$false
			  } else {
				$true
		  ...
		  Script RemoveDefaultWebAppPool
		  {
			SetScript = {
			...
			TestScript = {
			  try {
					$s = Get-WebAppPoolState -Name "DefaultAppPool"
					return $false
				} catch {
					 return $true
				}
			...
			DependsOn = @("[Script]RemoveDefaultWebSite")
		  }
	}
} 
